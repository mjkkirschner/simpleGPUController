...
void shiftDataOut(unsigned long data)
{
  ...
  //loop all bits in long
  //or we'll need to 32 explicit read and writes.
  for (int bit_index = 31; bit_index >= 0; bit_index -= 1)
  { 
     //clock low
      PORTE &= ~(1<<4);
...
void loop()
{
  ...
  if (currentState == commandReadMode) {
    ...
    if (Serial.available() > 0)
    {
      ...
      shiftDataOut(extract_long(currentDataBuffer,0 + i));
     
      shiftDataOut(extract_long(currentDataBuffer,4 + i));
      
      shiftDataOut(extract_long(currentDataBuffer,8+i));
